...
class Eventos extends Controller{
    ...
    public function inicio(){
        $eventos = Evento::where('estado','0')->get();
        return view('evento.inicio', compact('eventos'));
    }
    ...
    public function asistencia(){
        return view('evento.asistencia.buscar');
    }

    public function asistencia_buscar(Request $request){
        $nro = $request->nro_doc;
        $tipo = $request->tip_doc;
        $certificado = 0;
        
        if($request->tipo_busqueda == 'correo'){
            $participante = User::where('correo_invitado',$request->correo)->get();
            if(isset($participante)){
                
                foreach($participante as $p){
                    $evento = Evento::all();
                    foreach($evento as $e){
                        //1. Buscar si tiene certificado como participante
                        if($e->certificado == '0'){
                            $e->evento_certificado = 'no';
                            //este evento no da certificado

                        }elseif($e->certificado == '1'){
                            $e->evento_certificado = 'todas';
                            //preguntar si tiene todas las requeridas

                        }elseif($e->certificado == '2'){
                            $e->evento_certificado = 'una_pordia';
                            //pregunta si tiene una por día

                        }elseif($e->certificado == '3'){
                            //solo un registro basta, aquí se le adjunta el certificado
                            $participante_evento = EventoUserAsistenteDetalle::where('id_user_asistencia',$p->id)->where('id_evento',$e->id)->first();
                            if(isset($participante_evento)){
                                $certificado ++;
                                $e->c_asistente = 'si';
                                $p->certificado = 'si';
                                $e->participante = $participante_evento;
                                
                            }else{
                                $e->c_asistente = 'no';
                                $p->certificado = 'no';
                                //no tiene registros de 
                                $titulo='No se encuentra asistencia';
                                $mensaje = 'Al parecer no ha asistido a algún evento aún, por que no se encuentran registros de asistencias con sus datos.';
                                //return view('evento.asistencia.mensaje',compact('titulo','mensaje'));
                            }
                        }
                        //verificar si tiene certificado como ponente u organizacion
                        $participante_staff = StaffDetalle::where('id_evento',$e->id)->where('id_user_ponente',$p->id)->get();
                        if($participante_staff->isEmpty()){
                            $p->certificado = 'no';
                        }else{
                            foreach($participante_staff as $s){
                                $actividad = Actividad::find($s->id_actividad);
                                $s->actividad = $actividad;
                            }
                            $certificado ++;
                            $p->certificado = 'si';
                            $e->c_staff = 'si';
                            $e->c_staff_d = $participante_staff;
                        }
                    }
                    $p->evento = $evento;
                }
                ...
                if($certificado<=0){
                    ...
                    //dd($evento);
                    return view('evento.asistencia.detalle',compact('participante'));
                }


            }else{
                $titulo='No existe participante';
                $mensaje = 'No se encuentra a un participante con los datos ingresados, vuelva a intenarlo.';
                return view('evento.asistencia.mensaje',compact('titulo','mensaje'));
            }

            
        }else{
            $participante = User::where('nro_documento',$request->nro_doc)->where('tipo_documento',$request->tip_doc)->take(1)->get();
            if(isset($participante)){    
                
                foreach($participante as $p){
                    $evento = Evento::all();
                    //verificar si tiene certificado participación 
                    foreach($evento as $e){
                        //1. Buscar si tiene certificado como participante
                        if($e->certificado == '0'){
                            $e->evento_certificado = 'no';
