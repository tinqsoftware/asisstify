@extends('layouts.app')

@section('content')

<section class="inner-page-banner inner-banner-8" data-bg-image="{{ URL::asset('image/banner/inner-banner8.jpg')}}">
    <div class="container">
        <div class="entry-banner-content">
        <div class="breadcrumb-area">
            <div class="entry-breadcrumb">
            <span>
                    <a href="/" class="home">Asisstify</a>
                </span>
            <em class="delimiter">/</em>
            <span class="current">Asistencias</span>
            </div>
        </div>
        <h1 class="entry-title">MIS ASISTENCIAS</h1>
        </div>
    </div>
</section>
    <!--=====================================-->
    <!--=     Inne Page Banner Area End     =-->
    <!--=====================================-->
    <!--=====================================-->
    <!--= Speaker Single Section Area Start =-->
    <!--=====================================-->
<section class="speaker-single-wrap pt-0" >
    <div class="container">
        <!--
            <div class="row">
                <div class="col-lg-2 col-12">
                    <div class="speaker-single-box">
                        <div class="figure-box wow fadeInLeft animated" data-wow-delay="0.3s" data-wow-duration="1s">
                            <img src="{{ URL::asset('image/speaker/46.jpg')}}" alt="Speaker">
                        </div>
                    </div>
                </div>
                <div class="col-lg-10 col-12">
                    <div class="speaker-single-box">
                        <div class="content-box wow fadeInRight animated" data-wow-delay="0.3s" data-wow-duration="1s">
                            <div>
                                <h2 class="title">Brooklyn Simmons</h2>
                                <div class="sub-title">Web Technologist</div>
                                <p class="description">Lorem ipsum dolor sit amet, consectetur adipiscing elit aliquam purus
                                    sit
                                    amet luctus venenatis, lectus magna fringilla urna the porttitor rhoncus dolor purus
                                    enim
                                    praesent elementum facilisis vel fringilla est ullamcorper eget nulla facilisi etiam
                                    dignissim as diam quis enim lobortis scelerisque fermentum dui faucibus in ornare quam
                                    viverra orci sagittis eu volutpat odio facilisis mauris sit amet massa vitae tortor
                                    condimentum lacinia quis vel eros is donec ac odio tempor orci dapibus ultrices in
                                    iaculis
                                    nunc sed commodo fringilla phasellus faucibus.</p>
                                <ul class="social">
                                    <li>
                                    <a target="_blank" href="https://www.facebook.com/radiustheme" class="facebook"><i
                                                        class="fab fa-facebook-f"></i></a>
                                    </li>
                                    <li>
                                    <a target="_blank" href="https://twitter.com/radiustheme" class="twitter"><i
                                                        class="fab fa-twitter"></i></a>
                                    </li>
                                    <li>
                                    <a target="_blank" href="https://www.facebook.com/radiustheme" class="linkedin"><i
                                                        class="fab fa-linkedin-in"></i></a>
                                    </li>
                                    <li>
                                    <a target="_blank" href="https://www.facebook.com/radiustheme" class="instagram"><i
                                                        class="fab fa-instagram"></i></a>
                                    </li>
                                    <li>
                                    <a target="_blank" href="https://www.pinterest.com/radius_theme/" class="pinterest"><i class="fab fa-pinterest-p"></i></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                
            </div>
        -->
        <div class="speaker-single-event-wrap mt-0">
            <div class="row">
                @foreach($participante as $p)
                    @if(isset($p->apellidos))
                        <h3 class="title"><b>Participante: </b>{{$p->name}}, {{$p->apellidos}}</h3>
                    @else
                        <h3 class="title"><b>Participante: </b>{{$p->name}}</h3>
                    @endif

                    
                        @foreach($p->evento as $e)
                            @if($e->certificado != 0 && (isset($e->c_staff_d) || isset($e->participante->id)))
                                <div class="col-lg-4 wow fadeInLeft animated pb-5" data-wow-delay="0.3s" data-wow-duration="1s">
                                    <div class="speaker-single-event-box">
                                        <div class="top-box">
                                            <h3 class="title">{{$e->nombre}}</h3>
                                            <div class="sub-title">{{date('d-m-Y', strtotime($e->fecha_inicio))}}</div>
                                        </div>
                                        <div class="bottom-box">
                                            @if(isset($e->c_staff_d))                                       
                                                @foreach($e->c_staff_d as $s)
                                                    @if($s->actividad->tipo_actividad < 3)
                                                        @if($s->tipo_ponente == '10')
                                                            <!--<a href="/certificado/10/{{$e->id}}/{{$p->id}}" class="day-number mb-3" style="background-color:#993333;" target="_blank"><i class="fa fa-file-pdf"></i> Ver certificado Organizador</a>-->
                                                            
                                                        @elseif($s->tipo_ponente == '11')
                                                            <!--<a href="/certificado/11/{{$e->id}}/{{$p->id}}" class="day-number mb-3" style="background-color:#996633;" target="_blank"><i class="fa fa-file-pdf"></i> Ver certificado Colaborador</a>-->
                                                            
                                                        @elseif($s->tipo_ponente == '3')
                                                        <!--listo-->
                                                            <!--<a href="/certificado/3/{{$s->actividad->id}}/{{$p->id}}" target="_blank" class="day-number mb-3" style="background-color:#998033;"><i class="fa fa-file-pdf"></i> Ver certificado Ponente: <span style="font-size:6;">{{$s->actividad->nombre_actividad}}</span></a>-->
                                                            
                                                        @elseif($s->tipo_ponente == '2')
                                                        <!--listo-->
                                                            <!--<a href="/certificado/2/{{$s->actividad->id}}/{{$p->id}}" target="_blank" class="day-number mb-3" style="background-color:#999933;"><i class="fa fa-file-pdf"></i> Ver certificado Moderador: <span style="font-size:6;">{{$s->actividad->nombre_actividad}}</span></a>-->
                                                            
                                                        @elseif($s->tipo_ponente == '4')
                                                            <!--<a href="/certificado/4/{{$s->actividad->id}}/{{$p->id}}" target="_blank" class="day-number mb-3" style="background-color:#809933;"><i class="fa fa-file-pdf"></i> Ver certificado Panelista: <pspan style="font-size:6;">{{$s->actividad->nombre_actividad}}</span></a>-->
                                                            
                                                        @elseif($s->tipo_ponente == '1')
                                                            <!--<a href="/certificado/1/{{$s->actividad->id}}/{{$p->id}}"  class="day-number mb-3" style="background-color:#639933;"><i class="fa fa-file-pdf"></i> Ver certificado Presentador: <span style="font-size:6;">{{$s->actividad->nombre_actividad}}</span></a>-->

                                                        @endif
                                                    @endif
                                                @endforeach
                                                <!--listo-->
                                                <a href="/certificado/8/{{$e->id}}/{{$p->id}}" target="_blank" class="day-number mb-3" style="background-color:#339933;"><i class="fa fa-file-pdf"></i> Ver certificado Asistente</a>
                                            @elseif(isset($e->participante->id))
                                            <!--listo-->
                                                <a href="/certificado/7/{{$e->id}}/{{$p->id}}" target="_blank" class="day-number mb-3" style="background-color:#339933;"><i class="fa fa-file-pdf"></i> Ver certificado Asistente</a>
                                            @endif
                                            
                                        </div>
                                        
                                    </div>
                                </div>
                            @endif
                        @endforeach
                    
                @endforeach
                <!--
                    <div class="col-lg-4 wow fadeInLeft animated" data-wow-delay="0.5s" data-wow-duration="1s">
                        <div class="speaker-single-event-box">
                            <div class="top-box">
                                <div class="day-number">Day - 2</div>
                            </div>
                            <div class="bottom-box">
                                <h3 class="title">July 25, 2021 12.00-13.00</h3>
                                <div class="sub-title">Conference Room 2</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 wow fadeInLeft animated" data-wow-delay="0.7s" data-wow-duration="1s">
                        <div class="speaker-single-event-box">
                            <div class="top-box">
                                <div class="day-number">Day - 3</div>
                            </div>
                            <div class="bottom-box">
                                <h3 class="title">July 24, 2021 12.00-13.00</h3>
                                <div class="sub-title">Conference Room 3</div>
                            </div>
                        </div>
                    </div>
                -->
            </div>
        </div>
    </div>
</section>
<div class="twice-section-bg"  data-bg-image="image/banner/banner3.jpg" >
    <section class="video-wrap-layout1">
        <div class="container">
          <div class="section-heading style-five mb-10">
            <h2 class="title">¿Quieres mejorar el control de <span>asistencia de tus eventos?</span></h2>
          </div>
          <div class="video-box-layout1 has-animation">
            <div class="figure-box">
              <img src="image/banner/video1.jpg" alt="Video" width="908" height="552">
              <a href="http://www.youtube.com/watch?v=2RH9YLYFCz4" class="play-btn play-btn-primary">
                <i class="fas fa-play"></i>
              </a>
            </div>
          </div>
        </div>
    </section>
    <section class="service-wrap-layout1">
        <div class="container">
        <div class="row">
            <div class="col-xl-4 col-md-6 col-12 wow fadeInUp animated" data-wow-delay="1.1s" data-wow-duration="1s">
                <div class="service-box-layout1 bg-style-two color-five">
                <div class="icon-box">
                    <svg width="51" height="52" viewBox="0 0 51 52" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_366:484)">
                        <path d="M6.53905 37.7603C6.62422 37.8442 6.72675 37.9091 6.83961 37.9496V50.5562C6.83961 51.0238 7.21872 51.4029 7.6863 51.4029H46.6342C47.1018 51.4029 47.4809 51.0238 47.4809 50.5562V30.2355C47.4809 29.768 47.1018 29.3888 46.6342 29.3888H42.3941C43.2524 28.2693 44.0941 26.9182 44.0941 26.0021C44.0933 24.5939 43.221 23.3334 41.9038 22.8361C40.5862 22.3387 39.0987 22.7083 38.1673 23.7642C37.2358 22.7083 35.7483 22.3387 34.4307 22.8361C33.1136 23.3334 32.2412 24.5939 32.2404 26.0021C32.2404 26.9182 33.0821 28.2693 33.9404 29.3888H16.1073L19.7095 25.7871L25.6963 19.7999L37.6711 7.82504C38.0015 7.49471 38.0015 6.9585 37.6711 6.62818L31.6843 0.640964C31.3482 0.323455 30.8228 0.323455 30.4871 0.640964L22.5973 8.53034C19.8323 6.36565 16.5369 5.01251 14.9204 6.62818C13.8596 7.68902 14.0832 9.47791 14.9737 11.3635L11.2757 15.0629C9.3892 14.1728 7.5999 13.9487 6.53905 15.0104C4.92422 16.624 6.27735 19.921 8.44246 22.6856L0.552254 30.5741C0.221514 30.9049 0.221514 31.4407 0.552254 31.7714L6.53905 37.7603ZM33.9338 26.0021C33.9338 25.0669 34.692 24.3087 35.6272 24.3087C36.5623 24.3087 37.3206 25.0669 37.3206 26.0021V26.2307C37.3206 26.6983 37.6997 27.0774 38.1673 27.0774C38.6348 27.0774 39.0139 26.6983 39.0139 26.2307V26.0021C39.0139 25.0669 39.7722 24.3087 40.7073 24.3087C41.6425 24.3087 42.4007 25.0669 42.4007 26.0021C41.9149 27.2754 41.1629 28.4301 40.1943 29.3888H36.1402C35.172 28.4301 34.4196 27.2754 33.9338 26.0021ZM31.3937 31.0822H45.7875V49.7095H31.3937V31.0822ZM24.6202 31.0822H29.7003V49.7095H24.6202V31.0822ZM22.9268 31.0822V49.7095H8.533V37.0091H8.48628L14.4598 31.0363V31.0822H22.9268ZM19.1117 23.9903L14.3218 19.2004L19.1117 14.4109L23.9012 19.2004L19.1117 23.9903ZM31.0857 2.43688L35.8752 7.22681L25.0985 18.0031L20.309 13.2169L31.0857 2.43688ZM16.1177 7.82504C16.3203 7.66215 16.5782 7.58443 16.8375 7.60758C17.7813 7.60758 19.4292 8.22482 21.3917 9.73547L18.5131 12.6141L18.0285 13.0991C15.9837 10.4449 15.5757 8.36704 16.1177 7.82504ZM15.8316 12.9052C16.1346 13.3811 16.4666 13.8519 16.8238 14.3084L14.2151 16.9171C13.7587 16.5599 13.2882 16.2279 12.8132 15.924L15.8316 12.9052ZM7.73633 16.2076C7.93849 16.0447 8.19564 15.9666 8.45445 15.9898C9.39912 15.9898 11.0479 16.6062 13.0104 18.1177L12.5258 18.6018L9.64718 21.4809C7.60155 18.8263 7.19433 16.7484 7.73633 16.2076ZM13.1245 20.3977L17.9144 25.1876L7.13769 35.9644L2.3523 31.1744L13.1245 20.3977Z" />
                        <path d="M44.941 11.9105C44.0096 10.8546 42.5221 10.485 41.2045 10.9824C39.8873 11.4797 39.015 12.7403 39.0142 14.1484C39.0142 15.9873 43.0124 20.9219 44.941 20.9219C46.8696 20.9219 50.8679 15.9873 50.8679 14.1484C50.867 12.7403 49.9947 11.4797 48.6775 10.9824C47.36 10.485 45.8725 10.8546 44.941 11.9105ZM49.1745 14.1484C49.1559 15.21 45.9002 19.1041 44.9547 19.2285C43.9819 19.1033 40.7262 15.2092 40.7075 14.1484C40.7075 13.2132 41.4658 12.455 42.4009 12.455C43.3361 12.455 44.0943 13.2132 44.0943 14.1484V14.377C44.0943 14.8446 44.4734 15.2237 44.941 15.2237C45.4086 15.2237 45.7877 14.8446 45.7877 14.377V14.1484C45.7877 13.2132 46.5459 12.455 47.4811 12.455C48.4163 12.455 49.1745 13.2132 49.1745 14.1484Z" />
                        <path d="M44.941 3.14136C44.4734 3.14136 44.0943 3.52047 44.0943 3.98805V4.83474H43.2476C42.78 4.83474 42.4009 5.21385 42.4009 5.68144C42.4009 6.14902 42.78 6.52813 43.2476 6.52813H44.0943V7.37482C44.0943 7.84241 44.4734 8.22152 44.941 8.22152C45.4085 8.22152 45.7877 7.84241 45.7877 7.37482V6.52813H46.6343C47.1019 6.52813 47.481 6.14902 47.481 5.68144C47.481 5.21385 47.1019 4.83474 46.6343 4.83474H45.7877V3.98805C45.7877 3.52047 45.4085 3.14136 44.941 3.14136Z" />
                        <path d="M33.9341 16.6885C33.9341 17.1561 33.555 17.5352 33.0874 17.5352C32.6198 17.5352 32.2407 17.1561 32.2407 16.6885C32.2407 16.2209 32.6198 15.8418 33.0874 15.8418C33.555 15.8418 33.9341 16.2209 33.9341 16.6885Z" />
                        <path d="M26.3135 27.6954C26.7811 27.6954 27.1602 27.3163 27.1602 26.8487V26.002H28.0069C28.4745 26.002 28.8536 25.6229 28.8536 25.1553C28.8536 24.6877 28.4745 24.3086 28.0069 24.3086H27.1602V23.4619C27.1602 22.9943 26.7811 22.6152 26.3135 22.6152C25.8459 22.6152 25.4668 22.9943 25.4668 23.4619V24.3086H24.6201C24.1525 24.3086 23.7734 24.6877 23.7734 25.1553C23.7734 25.6229 24.1525 26.002 24.6201 26.002H25.4668V26.8487C25.4668 27.3163 25.8459 27.6954 26.3135 27.6954Z" />
                        <path d="M49.1744 25.1555C49.1744 25.6231 48.7952 26.0022 48.3277 26.0022C47.8601 26.0022 47.481 25.6231 47.481 25.1555C47.481 24.6879 47.8601 24.3088 48.3277 24.3088C48.7952 24.3088 49.1744 24.6879 49.1744 25.1555Z" />
                    </g>
                    <defs>
                        <clipPath id="clip0_366:484">
                        <rect width="51" height="51" fill="white" transform="translate(0 0.402832)" />
                        </clipPath>
                    </defs>
                    </svg>
                </div>
                <h3 class="title">Asistencia Automatizada</h3>
                <p class="description">Autoregistro de asistencia con QR y seguimiento preciso en tiempo real, eliminando errores manuales y asegurando datos confiables.</p>
                </div>
            </div>
            <div class="col-xl-4 col-md-6 col-12 wow fadeInUp animated" data-wow-delay="0.9s" data-wow-duration="1s">
            <div class="service-box-layout1 bg-style-two color-four">
                <div class="icon-box">
                <svg width="51" height="52" viewBox="0 0 51 52" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_366:476)">
                    <path d="M45.1561 42.9028H36.2736C35.3938 42.9028 34.6798 42.1888 34.6798 41.3091C34.6798 40.4293 35.3938 39.7153 36.2736 39.7153H44.0383L47.1302 31.2153H3.86732L6.95919 39.7153H14.7261C15.6058 39.7153 16.3198 40.4293 16.3198 41.3091C16.3198 42.1888 15.6058 42.9028 14.7261 42.9028H5.84357C5.17419 42.9028 4.57494 42.4842 4.34544 41.8531L0.0954427 30.1656C-0.0809323 29.6768 -0.0108073 29.1328 0.288818 28.7057C0.586318 28.2807 1.07294 28.0278 1.59357 28.0278H49.4061C49.9267 28.0278 50.4133 28.2807 50.7108 28.7078C51.0104 29.1328 51.0806 29.679 50.9042 30.1677L46.6542 41.8552C46.4247 42.4842 45.8254 42.9028 45.1561 42.9028Z" />
                    <path d="M34.5332 51.4028C34.4524 51.4028 34.3717 51.3965 34.2888 51.3837C33.4197 51.2498 32.8225 50.436 32.9564 49.5668L34.7988 37.5903H16.2008L18.0432 49.5668C18.177 50.4381 17.5799 51.2498 16.7108 51.3837C15.8417 51.5261 15.0257 50.9226 14.8939 50.0513L12.7689 36.2388C12.6988 35.7798 12.8305 35.3102 13.1344 34.9575C13.4383 34.6047 13.8782 34.4028 14.3435 34.4028H36.656C37.1214 34.4028 37.5634 34.6068 37.8652 34.9596C38.1669 35.3123 38.3008 35.7798 38.2307 36.241L36.1057 50.0535C35.9867 50.8397 35.3067 51.4028 34.5332 51.4028Z" />
                    <path d="M47.8122 30.684C46.9325 30.684 46.2185 29.97 46.2185 29.0903C46.2185 24.8849 43.9702 20.9048 40.3514 18.7012C39.5991 18.2422 39.3611 17.2625 39.8201 16.5103C40.2791 15.758 41.2587 15.52 42.011 15.979C46.5712 18.7585 49.4039 23.7821 49.4039 29.0903C49.406 29.97 48.692 30.684 47.8122 30.684Z" />
                    <path d="M39.3125 19.5278C41.0729 19.5278 42.5 18.1007 42.5 16.3403C42.5 14.5799 41.0729 13.1528 39.3125 13.1528C37.5521 13.1528 36.125 14.5799 36.125 16.3403C36.125 18.1007 37.5521 19.5278 39.3125 19.5278Z" />
                    <path d="M25.5 17.4028C20.8122 17.4028 17 13.5906 17 8.90283C17 4.21508 20.8122 0.402832 25.5 0.402832C30.1877 0.402832 34 4.21508 34 8.90283C34 13.5906 30.1877 17.4028 25.5 17.4028ZM25.5 3.59033C22.5696 3.59033 20.1875 5.97458 20.1875 8.90283C20.1875 11.8311 22.5696 14.2153 25.5 14.2153C28.4304 14.2153 30.8125 11.8311 30.8125 8.90283C30.8125 5.97458 28.4304 3.59033 25.5 3.59033Z" />
                    <path d="M41.4375 31.2153C40.5578 31.2153 39.8438 30.5013 39.8438 29.6216C39.8438 25.8136 36.7455 22.7153 32.9375 22.7153H18.0625C14.2545 22.7153 11.1562 25.8136 11.1562 29.6216C11.1562 30.5013 10.4423 31.2153 9.5625 31.2153C8.68275 31.2153 7.96875 30.5013 7.96875 29.6216C7.96875 24.0562 12.4971 19.5278 18.0625 19.5278H32.9375C38.5029 19.5278 43.0312 24.0562 43.0312 29.6216C43.0312 30.5013 42.3172 31.2153 41.4375 31.2153Z" />
                    </g>
                    <defs>
                    <clipPath id="clip0_366:476">
                        <rect width="51" height="51" fill="white" transform="translate(0 0.402832)" />
                    </clipPath>
                    </defs>
                </svg>
                </div>
                <h3 class="title">Agenda Diaria del Evento</h3>
                <p class="description">Los asistentes de tus eventos podrán ver el detalle de cada actividad (nombre, hora, lugar, ponentes, etc).</p>
            </div>
            </div>
            
            <div class="col-xl-4 col-md-6 col-12 wow fadeInUp animated" data-wow-delay="1.3s" data-wow-duration="1s">
            <div class="service-box-layout1 bg-style-two color-six">
                <div class="icon-box">
                <svg width="64" height="65" viewBox="0 0 64 65" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_366:522)">
                    <path d="M46.5363 6.74756H3.60388C1.61663 6.74756 0 8.36419 0 10.3513V36.1084C0 38.0955 1.61663 39.7122 3.60388 39.7122H6.38776V49.301C6.38776 49.9115 6.75551 50.4621 7.31976 50.6959C7.50652 50.7734 7.70277 50.8109 7.89727 50.8109C8.29027 50.8109 8.67627 50.6574 8.96514 50.3686L19.6215 39.7124H21.7595C22.5934 39.7124 23.2693 39.0365 23.2693 38.2026C23.2693 37.3686 22.5932 36.6927 21.7595 36.6927C21.7595 36.6927 18.9233 36.6947 18.8937 36.6977C18.5422 36.7216 18.1972 36.8664 17.9284 37.135L9.40727 45.6561C9.40727 45.6561 9.40464 38.1071 9.39977 38.068C9.33164 37.2972 8.68514 36.6927 7.89677 36.6927H3.60388C3.28163 36.6927 3.01963 36.4306 3.01963 36.1085V10.3513C3.01963 10.0291 3.28176 9.76706 3.60388 9.76706H46.5363C46.8586 9.76706 47.1206 10.0292 47.1206 10.3513V20.2332C47.1206 21.0671 47.7967 21.743 48.6303 21.743C49.464 21.743 50.1401 21.0671 50.1401 20.2332V10.3513C50.1401 8.36419 48.5235 6.74756 46.5363 6.74756Z" />
                    <path d="M60.3965 25.2144H30.3446C28.3575 25.2144 26.7407 26.831 26.7407 28.8182V46.3895C26.7407 48.3767 28.3573 49.9934 30.3446 49.9934H49.1589L56.7814 57.6159C57.0703 57.9048 57.4564 58.0582 57.8493 58.0582C58.0438 58.0582 58.24 58.0205 58.4268 57.9432C58.9908 57.7094 59.3588 57.159 59.3588 56.5483V49.9933H60.3964C62.3835 49.9933 64.0003 48.3767 64.0003 46.3894V28.8181C64.0004 26.831 62.3838 25.2144 60.3965 25.2144ZM60.9809 46.3895C60.9809 46.7117 60.7188 46.9738 60.3967 46.9738H57.8497C57.0552 46.9738 56.4054 47.5878 56.3458 48.3672C56.3419 48.4002 56.3394 52.9034 56.3394 52.9034L50.8576 47.4215C50.8519 47.4158 50.8459 47.4104 50.8401 47.4047C50.8186 47.3837 50.7983 47.3654 50.779 47.349C50.5133 47.116 50.1661 46.9737 49.7851 46.9737H30.3446C30.0224 46.9737 29.7604 46.7115 29.7604 46.3894V28.8181C29.7604 28.4959 30.0225 28.2339 30.3446 28.2339H60.3967C60.7188 28.2339 60.9809 28.496 60.9809 28.8181V46.3895Z" />
                    <path d="M45.3706 36.0942C44.5372 36.0942 43.8608 36.7706 43.8608 37.604C43.8608 38.4374 44.5372 39.1138 45.3706 39.1138C46.2041 39.1138 46.8803 38.4374 46.8803 37.604C46.8803 36.7706 46.204 36.0942 45.3706 36.0942Z" />
                    <path d="M38.8093 16.7292H11.3315C10.4978 16.7292 9.82178 17.4051 9.82178 18.239C9.82178 19.0729 10.4979 19.7488 11.3315 19.7488H38.8093C39.6431 19.7488 40.3191 19.0729 40.3191 18.239C40.3191 17.4051 39.6431 16.7292 38.8093 16.7292Z" />
                    <path d="M21.7597 26.7109H11.3315C10.4978 26.7109 9.82178 27.3868 9.82178 28.2207C9.82178 29.0546 10.4979 29.7304 11.3315 29.7304H21.7597C22.5936 29.7304 23.2694 29.0546 23.2694 28.2207C23.2694 27.3868 22.5934 26.7109 21.7597 26.7109Z" />
                    <path d="M38.355 36.0942C37.5217 36.0942 36.8452 36.7706 36.8452 37.604C36.8452 38.4374 37.5217 39.1138 38.355 39.1138C39.1885 39.1138 39.8647 38.4374 39.8647 37.604C39.8647 36.7706 39.1885 36.0942 38.355 36.0942Z" />
                    <path d="M52.3848 36.0942C51.5514 36.0942 50.875 36.7706 50.875 37.604C50.875 38.4374 51.5514 39.1138 52.3848 39.1138C53.2183 39.1138 53.8945 38.4374 53.8945 37.604C53.8945 36.7706 53.2181 36.0942 52.3848 36.0942Z" />
                    </g>
                    <defs>
                    <clipPath id="clip0_366:522">
                        <rect width="64" height="64" fill="white" transform="translate(0 0.402832)" />
                    </clipPath>
                    </defs>
                </svg>
                </div>
                <h3 class="title">Generación Instantánea de Certificados</h3>
                <p class="description">Certificados listos para descargar y compartir, mejorando la experiencia de los asistentes.</p>
            </div>
            </div>
        </div>
        </div>
    </section>
    <section class="newsletter-wrap-layout1" style="z-index: 1;">
        <div class="container">
        <div class="newsletter-box-layout1 wow fadeInUp animated" data-wow-delay="0.5s" data-wow-duration="0.75s">
            <div class="shape-box">
            <svg class="wow fadeInDown animated" data-wow-delay="1.2s" data-wow-duration="0.5s" width="201" height="191" viewBox="0 0 201 191" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M152.65 60.0388C148.702 53.2008 140.282 50.5247 133.196 53.5571L113.382 19.2374C112.168 17.1361 109.778 16.1437 107.433 16.7683C105.089 17.3932 103.509 19.4425 103.502 21.8685C103.454 38.5268 96.9571 53.6446 84.1936 66.8024C74.542 76.7519 64.7388 81.7774 64.6436 81.8249C64.4992 81.8976 64.364 81.9845 64.2343 82.0779L39.8186 96.1743C27.5532 103.256 23.3357 118.996 30.4171 131.261C37.0871 142.814 51.4374 147.223 63.3317 141.779L88.1796 167.81C91.4696 171.257 96.6494 172 100.776 169.618L105.887 166.667C108.641 165.077 110.479 162.36 110.931 159.212C111.383 156.065 110.382 152.94 108.187 150.64L86.8812 128.32L89.915 126.569C90.0614 126.503 90.2048 126.429 90.341 126.34C90.4326 126.28 99.6858 120.302 113.128 116.919C130.905 112.444 147.245 114.377 161.696 122.664C163.377 123.628 165.35 123.603 166.949 122.68C167.354 122.446 167.733 122.156 168.079 121.81C169.792 120.091 170.128 117.525 168.915 115.424L149.1 81.1036C155.27 76.4834 157.162 67.8538 153.214 61.0159L152.65 60.0388ZM100.913 151.91L103.746 154.879C104.633 155.808 105.037 157.069 104.854 158.341C104.672 159.611 103.93 160.709 102.818 161.351L97.7067 164.301C96.0403 165.264 93.9488 164.963 92.6204 163.572L68.8876 138.709L81.41 131.479L87.7451 138.116L86.4745 138.85C85.0063 139.697 84.5034 141.574 85.351 143.043C86.1986 144.511 88.0757 145.014 89.5439 144.166L92.1071 142.686L96.5502 147.34L95.7171 147.821C94.249 148.669 93.746 150.546 94.5936 152.014C95.4413 153.482 97.3184 153.985 98.7865 153.138L100.913 151.91ZM77.9453 111.351L61.4257 120.888C59.9576 121.736 59.4546 123.613 60.3022 125.081C61.1499 126.549 63.027 127.052 64.4951 126.205L81.0147 116.667L84.4579 122.631L80.5746 124.873C80.5244 124.899 80.474 124.923 80.4246 124.952L62.4311 135.34C62.4058 135.355 62.383 135.373 62.358 135.388C53.0351 140.716 41.1079 137.5 35.7335 128.192C30.3445 118.858 33.5541 106.88 42.888 101.491L64.9113 88.7755L77.9453 111.351ZM129.357 59.1862L142.306 81.6143L162.099 115.897C146.98 108.107 129.527 106.414 111.421 111.019C101.396 113.568 93.6676 117.383 89.7678 119.551L84.7963 110.94L70.2341 85.7172C74.0617 83.4234 81.2297 78.6378 88.4495 71.231C101.49 57.8527 108.751 41.8918 109.564 24.9038L129.357 59.1862ZM147.897 64.0856C150.149 67.9862 149.25 72.8555 145.995 75.7272L136.3 58.9344C140.414 57.5515 145.081 59.2079 147.333 63.1085L147.897 64.0856Z" />
                <path d="M144.774 44.5595C145.412 44.1907 145.921 43.5918 146.163 42.8339L149.798 31.4248C150.313 29.8096 149.421 28.0832 147.806 27.5683C146.19 27.0538 144.464 27.9459 143.949 29.561L140.314 40.9696C139.799 42.5848 140.691 44.3113 142.306 44.8261C143.164 45.0992 144.052 44.9763 144.774 44.5595Z" />
                <path d="M164.871 75.237C163.214 74.8754 161.579 75.9244 161.217 77.5805C160.855 79.2366 161.904 80.8725 163.56 81.2344L175.258 83.7903C176.036 83.96 176.809 83.819 177.448 83.4502C178.17 83.0334 178.72 82.3256 178.912 81.4468C179.274 79.7907 178.225 78.1548 176.569 77.7928L164.871 75.237Z" />
                <path d="M162.323 51.5605L156.436 54.9594C154.968 55.8071 154.465 57.6842 155.313 59.1523C156.16 60.6204 158.037 61.1234 159.506 60.2758L165.393 56.8769C166.861 56.0293 167.364 54.1521 166.516 52.684C165.669 51.2159 163.791 50.7129 162.323 51.5605Z" />
                <path d="M53.8765 127.437C53.7164 127.313 53.545 127.206 53.3664 127.118C53.1837 127.029 52.9964 126.956 52.8035 126.904C52.608 126.854 52.408 126.821 52.2065 126.809C52.0077 126.797 51.806 126.803 51.6081 126.829C51.4076 126.856 51.211 126.902 51.0236 126.964C50.6408 127.093 50.2872 127.297 49.9842 127.564C49.834 127.697 49.6957 127.844 49.5746 128.003C49.4535 128.161 49.3442 128.334 49.2559 128.513C49.165 128.693 49.0939 128.883 49.0422 129.076C48.9905 129.269 48.9571 129.469 48.9459 129.67C48.933 129.869 48.9397 130.071 48.9668 130.271C48.9924 130.469 49.0386 130.666 49.1023 130.856C49.1659 131.046 49.2486 131.232 49.3484 131.405C49.4481 131.578 49.568 131.742 49.7024 131.895C49.8336 132.043 49.9808 132.181 50.1409 132.305C50.2994 132.426 50.4708 132.533 50.6494 132.621C50.8295 132.712 51.0194 132.783 51.2123 132.835C51.4051 132.886 51.6052 132.92 51.8081 132.934C52.0069 132.946 52.2113 132.938 52.4092 132.913C52.6071 132.887 52.8037 132.841 52.9938 132.777C53.1865 132.712 53.3685 132.628 53.5413 132.528C53.7167 132.427 53.8803 132.311 54.0331 132.177C54.1833 132.044 54.3189 131.899 54.4427 131.739C54.5638 131.58 54.6704 131.409 54.7587 131.23C54.8481 131.047 54.9193 130.857 54.9736 130.663C55.0253 130.47 55.056 130.272 55.0699 130.069C55.0828 129.87 55.0761 129.668 55.0505 129.47C55.0234 129.27 54.9772 129.073 54.915 128.886C54.8498 128.693 54.7645 128.508 54.6648 128.335C54.565 128.163 54.4478 127.996 54.315 127.846C54.1822 127.696 54.035 127.558 53.8765 127.437Z" />
            </svg>
            </div>
            <h2 class="title">¿Quieres probar la plataforma?</h2>
            <div class="newsletter-form">
            <div class="input-group stylish-input-group">
                <a href="/contactanos">
                    <span class="input-group-addon">
                        <button ><span>Clic aquí para contactarte!</span></button>
                    </span>
                </a>
            </div>
            </div>
        </div>
        </div>
    </section>
</div>
@endsection
